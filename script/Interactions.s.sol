...
contract Interactions is Script {
    ...
    function run() external {
        address loyaltyProgramAddress = 0x948B3c65b89DF0B4894ABE91E6D02FE579834F8F; // DevOpsTools.get_most_recent_deployment("MockLoyaltyProgram", block.chainid);
        MockLoyaltyProgram loyaltyProgram = MockLoyaltyProgram(payable(loyaltyProgramAddress));
        address loyaltyGiftsAddress = 0xbdEd0D2bf404bdcBa897a74E6657f1f12e5C6fb6; // DevOpsTools.get_most_recent_deployment("PointsForLoyaltyGiftsAndVouchers", block.chainid);
        ...
        for (uint256 i = 0; GIFTS_TO_SELECT.length > i; i++) {
            MockLoyaltyProgram(loyaltyProgram).addLoyaltyGift(payable(loyaltyGiftsAddress), GIFTS_TO_SELECT[i]);
        }
        MockLoyaltyProgram(loyaltyProgram).mintLoyaltyVouchers(
        ...
        MockLoyaltyProgram(loyaltyProgram).safeTransferFrom(address2, address3, 1, 1, "");
        vm.stopBroadcast();

        // should also include claiming tokens to various cards WIP
    }

    function mintLoyaltyPoints(MockLoyaltyProgram lpInstance, uint256 numberOfPoints) public {
        vm.startBroadcast();
        MockLoyaltyProgram(lpInstance).mintLoyaltyPoints(numberOfPoints);
        vm.stopBroadcast();
    }

    function mintLoyaltyCards(MockLoyaltyProgram lpInstance, uint256 numberOfCards) public {
        vm.startBroadcast();
        MockLoyaltyProgram(lpInstance).mintLoyaltyCards(numberOfCards);
        vm.stopBroadcast();
    }

    function giftLoyaltyCard(MockLoyaltyProgram lpInstance, uint256 loyaltyCardId, address from, address to) public {
        vm.startBroadcast();
        MockLoyaltyProgram(lpInstance).safeTransferFrom(from, to, loyaltyCardId, 1, "");
        vm.stopBroadcast();
    }

    function giftLoyaltyPoints(MockLoyaltyProgram lpInstance, address from, uint256 loyaltyCardId, uint256 numberOfPoints)
        public
    {
        vm.startBroadcast();
        address to = MockLoyaltyProgram(lpInstance).getTokenBoundAddress(loyaltyCardId);
        MockLoyaltyProgram(lpInstance).safeTransferFrom(from, to, 0, numberOfPoints, "");
        vm.stopBroadcast();
    }

    function mintLoyaltyGifts(
        MockLoyaltyProgram lpInstance,
        address payable loyaltyGiftAddress,
        uint256[] memory loyaltyGiftIds,
        uint256[] memory numberOfVouchers
    ) public {
        vm.startBroadcast();
        MockLoyaltyProgram(lpInstance).mintLoyaltyVouchers(loyaltyGiftAddress, loyaltyGiftIds, numberOfVouchers);
        vm.stopBroadcast();
    }
}

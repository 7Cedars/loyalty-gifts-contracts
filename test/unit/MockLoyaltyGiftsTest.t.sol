...
contract PointsForLoyaltyGiftsAndVouchersTest is Test {
    /**
     * events
     */
    ...
    PointsForLoyaltyGiftsAndVouchers pointsForLoyaltyGiftsAndVouchers;

    function setUp() external {
        DeployPointsForLoyaltyGiftsAndVouchers deployer = new DeployPointsForLoyaltyGiftsAndVouchers();
        pointsForLoyaltyGiftsAndVouchers = deployer.run();
    }

    function testLoyaltyGiftHasTokenised() public {
        uint256[] memory tokenised = pointsForLoyaltyGiftsAndVouchers.getTokenised();
    ...
    function testDeployEmitsevent() public {
        ...
        pointsForLoyaltyGiftsAndVouchers = new PointsForLoyaltyGiftsAndVouchers();
    }
    ...
    function testRequirementsReturnTrueWhenMet() public {
        for (uint256 i = 0; i < GIFT_PRICES.length; i++) {
            ...
            (success) = pointsForLoyaltyGiftsAndVouchers.requirementsLoyaltyGiftMet(addressOne, i, GIFT_PRICES[i]);
        ...
        for (uint256 i = 0; i < GIFT_PRICES.length; i++) {
            vm.expectRevert(
                abi.encodeWithSelector(
                    ILoyaltyGift.LoyaltyGift__RequirementsNotMet.selector, address(pointsForLoyaltyGiftsAndVouchers), i
                )
            );

            pointsForLoyaltyGiftsAndVouchers.requirementsLoyaltyGiftMet(addressOne, i, GIFT_PRICES[i] - 1);
        }
    }
}
